...
| **Status** | ✅ done 2026-07-17 on Sonnet — `async headers()` added in `next.config.ts`. Enforcing immediately: `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Strict-Transport-Security` (2yr + preload). CSP shipped as `Content-Security-Policy-Report-Only` per the plan's own rollout guidance (log violations before enforcing) — allowlist covers `self` + the Supabase project origin (`http(s)`/`wss`, derived from `NEXT_PUBLIC_SUPABASE_URL` so it can't drift from `images.remotePatterns`) + `api.dicebear.com`; `script-src`/`style-src` carry `'unsafe-inline'` (Next.js App Router hydration/RSC inline scripts and inline `style=""` attributes need it — no nonce plumbing added, since that would mean threading a per-request nonce through the existing auth middleware, which is out of scope for this phase and riskier than the header change itself); `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`. Verified via local production build (`next start`) + curl: headers present on pages, redirects, and static assets (`/login`, `/`, `/management`, `/sw.js`); `npm run typecheck` and `npm run build` both green. **Follow-up (not done here):** watch the CSP report-only violations in production, then flip the header key to `Content-Security-Policy` to actually enforce it — this phase does not yet block anything via CSP, only logs. |
...
| **Status** | ✅ done 2026-07-17 on Fable — `refreshDataset` is now role-gated on the matching portal role (`full`→owner, `scheduler`→scheduler, `installer`→installer; composes with the Phase 2 RPC caller checks) and returns `AppDataset \| null`, with `null` on unauthorized/throttled so the realtime bridge skips `setData` instead of wiping the store (every caller already null-checked). Per-user token-bucket rate limiting added in `src/lib/rate-limit.ts` (in-memory per serverless instance — lightweight defense in depth, not a distributed limiter; bounded map, unit-tested incl. clock-skew and capacity-cap cases): dataset refresh = burst 10 + 1 token/3s sustained (mirrors the client's own 3s foreground throttle; the 120ms realtime debounce only fires on low-frequency triggers, so legit use never hits it); unit-detail refetch (`refreshUnitDetail` / `refreshSchedulerUnitDetail`, shared per-user key) = burst 30 + 1/s (generous because the scoped bridge subscribes to `windows` unfiltered — only tight loops get caught). Bounded fan-out confirmed intact (`SUPABASE_IN_CONCURRENCY = 4` in `supabase-chunking.ts`). Typecheck + build green; new rate-limit tests pass (the one failing repo test, `manufacturing-process.test.mts`, is a pre-existing extensionless-import resolution issue, unrelated). |
