...
| 2026-07-20 | **MF2 — Factory-portal freshness (DONE, code; needs migration apply + manual QA)** | New `useManufacturingFreshness(scheduleRefresh)` hook: one facility-wide realtime channel on `window_production_status` + `window_manufacturing_schedule`, each event feeding B1's coalesced refresh (shared timer with the screen's own mutations → still one refetch per burst, incl. own echoes + a reflow's many schedule-row writes). Backfills on re-subscribe (postgres_changes aren't replayed after a socket drop) and on tab-visibility regain (covers a backgrounded tablet). Wired into `CutterQueue`, `CutterProduction`, `ManufacturingRoleQueue`. **Then** deleted the three 400 ms filter-refresh effects (B1's deferred task 3) — filtering is client-side; freshness now comes from realtime. New migration `20260720120000_enable_realtime_manufacturing.sql` adds both tables to the `supabase_realtime` publication (guarded/idempotent; no RLS/schema change) — **must be applied before this works in prod**; without it the subscription is silent (the filter-refresh fallback is already removed, so unapplied = stale idle tablets). Scope note: deliberately does NOT subscribe to `units` (per assessment scope), so a *second* cutter's idle tablet won't see a unit another cutter moved to production until it navigates/refocuses — acceptable, rare. Gate: typecheck ✓, build ✓, lint ✓, tests 78/1 (pre-existing). **Manual QA pending:** two-browser (mark in cutter → idle QC queue updates ≤ ~2 s), network-drop → reconnect refreshes, 10-mark burst = one refetch. |
| 2026-07-20 | **B1 — Instant queue actions (DONE, code; manual QA pending)** | New `useCoalescedRefresh(delayMs=1500)` hook (trailing-edge, unmount-safe; MF2 will feed it from realtime). `moveUnitToProduction`/`moveUnitBackToQueue`: `revalidatePath` → `after()`; call sites (cutter-queue, cutter-production) drop the unit optimistically with rollback-on-failure. `cutter-production` mark-cut/undo: per-click `router.refresh()` → coalesced (10-tap burst = 1 refetch). `manufacturing-role-queue`: pushback now optimistic (row leaves + dialog closes, restore+toast on error) instead of immediate `refreshOnSuccess`; every success routes through the coalesced refresh. M5 server side: the 7 pushback/undo/issue reflows (`returnWindowToCutter/Assembler`, `undoWindowCut/Assembly/QC`, `markWindowManufacturingIssue`, `resolveWindowManufacturingIssue`) moved from inline `await reflowManufacturingSchedules` into `after()` via a shared `scheduleManufacturingReflow` helper (mirrors the mark path); `revalidateManufacturingPaths()` shrunk 6→5 (dropped `/management/settings`, untouched by a status change). No reflow-math change — only WHEN it runs (status is committed before response; parity holds). Filter-refresh effects deliberately NOT deleted (deferred to MF2). Owner settings/manual-shift reflows (updateManufacturingSettings, toggleManufacturingWorkday, shiftWindowManufacturingSchedule) left inline — out of the factory-interaction scope. Gate: typecheck ✓, build ✓, lint ✓ (changed files), tests 78/1 (pre-existing fail). **Manual QA still required** (I can't drive authed two-browser flows headlessly): tap-to-feedback <100 ms, 10-mark burst = 1 refetch (Network tab), two-browser reconciliation ≤ ~2 s, pushback rollback-on-error. |
| 2026-07-19 | **A5r — CSP enforce flip (DONE, gated on pre-merge soak)** | `next.config.ts`: `Content-Security-Policy-Report-Only` → `Content-Security-Policy`. Static audit backing the flip: only cross-origin host in `src/` is api.dicebear.com (already in img-src); no `dangerouslySetInnerHTML` / `javascript:` URIs / external scripts (the one `next/script` is same-origin `/cookie-purge.js`); SpeedInsights + Supabase realtime WS covered by `'self'`/supabase origins. Verified the enforced header emits correctly from `next start`. `'unsafe-inline'` retained for script+style (Next bootstrap + Tailwind/framer; no nonce pipeline — a nonce upgrade is out of scope). **Action required before merging to main:** confirm a clean browser-console CSP soak across all six portals; rollback = revert the header key. |
