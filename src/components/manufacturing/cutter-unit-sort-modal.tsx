...
import {
  CUTTER_UNIT_SORT_FIELD_LABELS,
  type CutterUnitSortField,
  type CutterUnitSortLevel,
} from "@/lib/cutter-unit-sort";

interface CutterUnitSortModalProps {
  draftLevels: CutterUnitSortLevel[];
  availableFields?: CutterUnitSortField[];
  onClose: () => void;
  onApply: (levels: CutterUnitSortLevel[]) => void;
  onChange: (levels: CutterUnitSortLevel[]) => void;
}

const allFieldOptions = Object.entries(CUTTER_UNIT_SORT_FIELD_LABELS).map(
  ([value, label]) => ({ value: value as CutterUnitSortField, label })
);

export function CutterUnitSortModal({
  draftLevels,
  availableFields,
  onClose,
  onApply,
  onChange,
}: CutterUnitSortModalProps) {
  const fieldOptions = availableFields
    ? allFieldOptions.filter((o) => availableFields.includes(o.value))
    : allFieldOptions;

  function addLevel() {
    if (draftLevels.length >= 3) return;
    const usedFields = new Set(draftLevels.map((l) => l.field));
    const nextField = fieldOptions.find((o) => !usedFields.has(o.value))?.value;
    if (!nextField) return;
  ...
  function updateLevel(idx: number, patch: Partial<CutterUnitSortLevel>) {
        ...
        <div className="space-y-2">
          {draftLevels.map((level, idx) => {
            const usedFields = new Set(
              draftLevels.filter((_, i) => i !== idx).map((l) => l.field)
            );
            const availableOptions = fieldOptions.filter((o) => !usedFields.has(o.value));
            return (
              <div
                key={idx}
              ...
              >
                ...
                <select
                  value={level.field}
                  onChange={(e) =>
                    updateLevel(idx, { field: e.target.value as CutterUnitSortField })
                  }
          ...
          {draftLevels.length < 3 && draftLevels.length < fieldOptions.length && (
            <button
              type="button"
              onClick={addLevel}
