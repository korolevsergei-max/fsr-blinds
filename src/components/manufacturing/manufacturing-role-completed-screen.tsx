...
type CompletedDayBucket = {
  dateKey: string | null;
  dayParts: { dayLabel: string; dayNumber: number } | null;
  units: GroupedUnit[];
  totalBlinds: number;
};
...
function getBucketDayParts(dateKey: string | null) {
...
function getCompletionDateKey(item: ManufacturingCompletedWindowItem): string | null {
  if (!item.roleCompletedAt) return null;
  const parsed = parseStoredDate(item.roleCompletedAt);
  if (!parsed) return null;
  return formatDateKey(parsed);
}

function groupUnitsWithinDay(items: ManufacturingCompletedWindowItem[]): GroupedUnit[] {
...
function buildCompletedBuckets(items: ManufacturingCompletedWindowItem[]): CompletedDayBucket[] {
  const byDate = new Map<string, ManufacturingCompletedWindowItem[]>();
  const noDateItems: ManufacturingCompletedWindowItem[] = [];

  for (const item of items) {
    const key = getCompletionDateKey(item);
    if (!key) {
  ...
  const buckets: CompletedDayBucket[] = sortedDates.map((dateKey) => {
    const dateItems = byDate.get(dateKey)!;
    return {
      dateKey,
      dayParts: getBucketDayParts(dateKey),
      units: groupUnitsWithinDay(dateItems),
      totalBlinds: dateItems.length,
    };
  });
  ...
  if (noDateItems.length > 0) {
    buckets.push({
      dateKey: null,
      dayParts: null,
      units: groupUnitsWithinDay(noDateItems),
      totalBlinds: noDateItems.length,
    });
  }
...
export function ManufacturingRoleCompletedScreen({
  ...
  const completedDateChoices = useMemo(
    () =>
      [...new Set(
        data.items
          .map((item) => getCompletionDateKey(item))
  ...
  return (
    ...
    >
      <div
        ref={headerRef}
        className="sticky top-0 z-30 border-b border-border bg-card/95 px-4 pt-5 pb-4 backdrop-blur-md"
      >
        ...
        <p className="mt-3 text-[12px] text-tertiary">
          {filteredItems.length} blind{filteredItems.length === 1 ? "" : "s"} completed · lifecycle dates and issue history for audit.
        </p>
      </div>
      ...
      <div className="space-y-4 px-4 pt-4">
        {dayBuckets.length === 0 ? (
          ...
          <section key={bucket.dateKey ?? "no-date"} className="relative">
            {bucket.dayParts ? (
              <StickyDayRail
                dayLabel={bucket.dayParts.dayLabel}
                dayNumber={bucket.dayParts.dayNumber}
                isToday={bucket.dateKey === todayKey}
                isPast={Boolean(bucket.dateKey && bucket.dateKey < todayKey)}
                taskCount={bucket.totalBlinds}
              />
            ) : (
              <div className="sticky top-[var(--schedule-sticky-top)] z-20 -mx-4 border-y border-border bg-card px-4 py-3">
                <div className="flex items-center justify-between gap-4 px-4">
                  ...
                  <span className="rounded-full border border-border bg-card px-2.5 py-1 text-[10px] font-medium text-tertiary">
                    {bucket.totalBlinds} blind{bucket.totalBlinds === 1 ? "" : "s"}
                  </span>
                </div>
              </div>
            )}
            ...
            <div className="pt-3">
              <div className="flex flex-col gap-2">
                {bucket.units.map((unit) => (
                  <div key={`${bucket.dateKey ?? "no-date"}-${unit.unitId}`} className="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card">
                    <button
