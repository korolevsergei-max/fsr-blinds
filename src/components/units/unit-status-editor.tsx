...
type UnitStatusEditorProps = {
  ...
  /** Base path for the role, e.g. "/management/units" or "/scheduler/units" */
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-[11px] ${met ? "text-emerald-600" : "text-amber-600"}`}>
          {hint}
        </p>
      </div>
      {met ? (
...
export function UnitStatusEditor({ data, mediaItems, unitsBasePath }: UnitStatusEditorProps) {
  ...
  // Only allow advancing to the immediate next status — same rule as installer/management
  const allowedNext = UNIT_STATUSES.filter(
    (s) => UNIT_STATUS_ORDER[s] === currentStep + 1
  );

  function isLocked(status: UnitStatus): boolean {
    if (status === "bracketed_measured") return !criteria.ready;
    return false;
  }

  function lockReason(status: UnitStatus): string | null {
    ...
    if (!criteria.allMeasured)
      missing.push(
        `${criteria.totalWindows - criteria.measuredWindows} window${
          criteria.totalWindows - criteria.measuredWindows !== 1 ? "s" : ""
        } not measured`
      );
  ...
  const blockedByMeasurement = selectedStatus === "bracketed_measured" && !criteria.ready;
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await updateUnitStatus(
        unit.id,
        selectedStatus as Parameters<typeof updateUnitStatus>[1],
        note
      );
      if (!result.ok) {
  ...
  return (
    <div className="flex flex-col min-h-[100dvh]">
      ...
      <div className="flex-1 px-4 py-5 flex flex-col gap-6">
        ...
        {/* Bracketing exit criteria — shown when unit is at scheduled_bracketing */}
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <CriterionRow
                ...
                hint={
                  criteria.hasRooms
                    ? `${criteria.rooms.length} room${criteria.rooms.length !== 1 ? "s" : ""} in system`
                    : "No rooms added yet"
                }
              />
              <CriterionRow
